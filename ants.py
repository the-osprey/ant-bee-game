...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        """
        if insect.is_ant:
            if not insect.imposter and isinstance(insect, QueenAnt): #can't delete queen!
                return None
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # Phase 4: Special handling for Ninja
        # BEGIN Problem 8
        if not self.place.ant or (self.place.ant and self.place.ant.blocks_path == False):
            return False
...
class ThrowerAnt(Ant):
    ...
    food_cost = 3
    min_range = 0
    max_range = 10000000 #CAREFUL! notice AntColony.dimensions could be larger, but highly doubtful. Cant set to AntColony.dimensions
    ...
    def nearest_bee(self, hive):
        ...
        """
        leaf_location, throw_distance = self.place, 0
        while leaf_location is not hive: #leaf moves through spots until hits hive (or ant)
            if leaf_location.bees and throw_distance >= self.min_range and throw_distance < self.max_range: #changed if random_or_none(leaf_location.bees) is not None to just check if leaf_location.bees
                return random_or_none(leaf_location.bees)
            leaf_location, throw_distance = leaf_location.entrance, throw_distance + 1
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        self.armor -= amount
        if self.armor <= 0:
            for x in list(self.place.bees): #remember: "If modify the sequence you iterate over while inside the loop, first make a copy."
...
class LongThrower(ThrowerAnt):
    ...
    name = 'Long'
    min_range = 5
    max_range = 100000000 #absurdely large number; see note before
    food_cost = 2
    implemented = True   # Change to True to view in the GUI
...
class WallAnt(Ant):
    """WallAnt has strong armor to block bees"""
    name = 'Wall'
    food_cost = 4
    implemented = True
    def __init__(self):
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        """
        if self.imposter: #kill imposter Queens first!
            self.reduce_armor(self.armor)
            self.place.remove_insect(self)
        else: #normal queen action
            room_behind = self.place.exit #set tunnel 'room' behind queen ant
            while room_behind is not None: #move back through rooms until hit end of tunnel aka None
                if room_behind.ant is not None:
                    if room_behind.ant not in self.powered_ants: #double normal ant if hasnt been doubled
                        self.powered_ants.append(room_behind.ant)
                        room_behind.ant.damage *= 2
                    if room_behind.ant.container and room_behind.ant.ant not in self.powered_ants and room_behind.ant.ant: #check for container ants
                        self.powered_ants.append(room_behind.ant.ant)
                        room_behind.ant.ant.damage *= 2
                room_behind = room_behind.exit #move to next room
...
def make_slow(action):
    ...
    """
    def slower(colony):
        if colony.time % 2 == 0: #slow time
            return action(colony)
    return slower

def make_stun(colony):
...
def apply_effect(effect, bee, duration):
    ...
    normal_action, modified_action = bee.action, effect(bee.action)
    def applier(colony):
        nonlocal duration #need to make nonlocal so it reduces each time fxn applied (like bank account withdrawing ex)
        if duration != 0: #aka if bee has been hit
            duration -= 1
            return modified_action(colony)
        else:
            return normal_action(colony)
    bee.action = applier #change bee action to affected action
    return bee.action
